    ...
    for proc in processes:
        ...
        for source in sources:
            if not source.exists():
                raise Exception(f"Specified path 'f{source.absolute()}' is not found for process '{proc_id}'")

            manifests_files += chain(source.glob("*.yml"), source.glob("*.yaml"))
            if not manifests_files:
                logging.warning("No manifests found for process '%s' in '%s'", proc_id, source.absolute())
        ...
        for manifest_file in manifests_files:
            ...
            kind = yml.get("kind")
            if not kind:
                raise Exception(f"Manifest file '{manifest_file}' for process '{proc_id}' is malformed, required field `kind` is not found")

            spec = yml.get("spec")
            if not spec:
                raise Exception(f"Manifest file '{manifest_file}' for process '{proc_id}' is malformed, required field `spec` is not found")
